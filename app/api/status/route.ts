...
<style>
:root{--bg:#0a0a0a;--card:#111;--border:#1a1a1a;--text:#e5e5e5;--muted:#737373;--green:#22c55e;--green-bg:#22c55e1a;--red:#ef4444;--red-bg:#ef44441a;--amber:#f59e0b;--amber-bg:#f59e0b1a}
*{box-sizing:border-box}
body{margin:0;padding:0;background:var(--bg);color:var(--text);font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,sans-serif;font-size:14px;line-height:1.6;display:flex;justify-content:center;align-items:center;min-height:100vh}
.container{width:100%;max-width:640px;padding:24px}
.header{text-align:center;margin-bottom:32px}
.header h1{margin:0 0 8px;font-size:28px;font-weight:700;letter-spacing:-0.02em}
.header p{margin:0;color:var(--muted)}
.overall{border-radius:16px;padding:32px;text-align:center;margin-bottom:24px;transition:all .3s ease}
.overall.ok{background:var(--green-bg);border:1px solid #22c55e33}
.overall.error{background:var(--red-bg);border:1px solid #ef444433}
.overall svg{width:64px;height:64px;margin-bottom:16px}
.overall h2{margin:0 0 8px;font-size:22px}
.overall .meta{color:var(--muted);font-size:13px}
.checks{display:flex;flex-direction:column;gap:12px}
.check{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:16px;display:flex;align-items:center;justify-content:space-between;transition:background .2s}
.check:hover{background:#161616}
.check-left{display:flex;align-items:center;gap:12px}
.check-left svg{width:20px;height:20px;flex-shrink:0}
.check-title{font-weight:600}
.check-desc{color:var(--muted);font-size:12px}
...
.latency{color:var(--muted);font-size:12px;margin-right:8px}
.loading{text-align:center;padding:48px}
.spinner{width:32px;height:32px;border:2px solid var(--border);border-top-color:var(--text);border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 16px}
@keyframes spin{to{transform:rotate(360deg)}}
.error-box{background:var(--red-bg);border:1px solid #ef444433;border-radius:12px;padding:24px;text-align:center}
.error-box button{margin-top:16px;background:var(--text);color:var(--bg);border:none;padding:8px 20px;border-radius:8px;font-weight:600;cursor:pointer}
...
function render(data){
  ...
  document.getElementById('overall').innerHTML='<div class="overall '+(ok?'ok':'error')+'"><div style="display:flex;justify-content:center">'+(ok?iOk():iErr())+'</div><h2>'+(ok?'All Systems Operational':'Some Systems Degraded')+'</h2><p class="meta">Last updated: '+new Date(data.timestamp).toLocaleString()+' · Version: '+(data.version||'dev')+' · Env: '+(data.environment||data.nodeEnv||'unknown')+'</p><p class="meta">Response time: '+data.totalLatencyMs+'ms</p></div>';
  let h='<div class="checks">';
  for(const[k,c]of Object.entries(data.checks)){
    const icon=c.status==='ok'?iOk():c.status==='missing'?iWarn():iErr();
    const bc=c.status==='ok'?'ok':c.status==='missing'?'missing':'error';
    const bt=c.status==='ok'?'Operational':c.status==='missing'?'Missing':'Error';
    h+='<div class="check"><div class="check-left">'+icon+'<div><div class="check-title">'+(labels[k]||k)+'</div><div class="check-desc">'+(descs[k]||'')+'</div></div></div><div style="display:flex;align-items:center">'+(c.latencyMs?'<span class="latency">'+c.latencyMs+'ms</span>':'')+'<span class="badge '+bc+'">'+bt+'</span></div></div>';
  }
  h+='</div>';
